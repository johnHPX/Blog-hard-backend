...
type responseCommentRepositoryInterface interface {
	Store(entity *model.ResponseComment) error
	List(commentID string, offset, limit, page int) ([]model.ResponseComment, error)
	Count(commentID string) (int, error)
	ListUser(userID string, offset, limit, page int) ([]model.ResponseComment, error)
	CountUser(userID string) (int, error)
	Update(entity *model.ResponseComment) error
	Remove(responseCommentID string) error
}

type responseCommentRepositoryImpl struct{}

func (r *responseCommentRepositoryImpl) scanIterator(rows *sql.Rows) (*model.ResponseComment, error) {
	ID := sql.NullString{}
...
func (r *responseCommentRepositoryImpl) Store(entity *model.ResponseComment) error {
...
func (r *responseCommentRepositoryImpl) List(commentID string, offset, limit, page int) ([]model.ResponseComment, error) {
	db, err := databaseConn.Connect()
	if err != nil {
		return nil, err
	}
...
`, limit, page, limit, offset)
	...
	responseComments := make([]model.ResponseComment, 0)
	for rows.Next() {
...
func (r *responseCommentRepositoryImpl) ListUser(userID string, offset, limit, page int) ([]model.ResponseComment, error) {
	db, err := databaseConn.Connect()
	if err != nil {
		return nil, err
	}
...
func (r *responseCommentRepositoryImpl) Update(entity *model.ResponseComment) error {
	db, err := databaseConn.Connect()
	if err != nil {
		return err
	}
